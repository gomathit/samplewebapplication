...
<P>This code was taken from the <i>Fish and Chips Club</i> with minor modifications. 
In the current case, there is only one code table. This may be overkill. However, in most 
practical apps, there will be many such code tables.  
...
<h3>Startup</h3>
 ...
 {@link hirondelle.predict.main.codes.CodeTableUtil#init(ServletContext)} to initialize all items 
...
{@link hirondelle.predict.main.codes.CodeTable#getTableName()} as key. 
 ...
 <P>Here is an example which references a code table named <tt>'outcomes'</tt> :<br>
<PRE>
  &lt;select name="Outcome"&gt;
...
This will avoid double-escaping of special characters, which is always undesirable.
 ...
 full <tt>Code</tt> object, using {@link hirondelle.predict.main.codes.CodeTable#codeFor(Id, CodeTable)}.</span>  
